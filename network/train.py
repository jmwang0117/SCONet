...
from network.common.optimizer import build_optimizer, build_scheduler
...
def parse_args():
  parser = argparse.ArgumentParser(description='SCONet training')
  parser.add_argument(
    '--cfg',
    dest='config_file',
    default='',
  ...
  with torch.no_grad():
    ...
    if IoU_1_1 > _cfg._dict['OUTPUT']['BEST_METRIC']:
      logger.info('=> Best metric on validation set encountered: ({} > {})'.
                  format(IoU_1_1, _cfg._dict['OUTPUT']['BEST_METRIC']))
      _cfg._dict['OUTPUT']['BEST_METRIC'] = IoU_1_1.item()
      checkpoint_info['best-metric'] = 'BEST_METRIC'
      metrics.update_best_metric_record(IoU_1_1, IoU_1_1, epoch_loss.item(), epoch)
